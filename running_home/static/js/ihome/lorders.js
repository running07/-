    ...
    $.get("/api/order/my?role=landlord", function(data){
        if ("0" == data.errcode) {
            ...
            $(".order-reply").on("click", function(){
                var orderId = $(this).parents("li").attr("order-id");
                $(".modal-reply").attr("order-id", orderId);
                console.log('insert1')
            });
                ...
                $.ajax({
                    ...
                    success:function (data) {
                        if ("4101" == data.errcode) {
                            ...
                            $(".orders-list>li[order-id="+ orderId +"]>div.order-content>div.order-text>ul li:eq(4)>span").html("已回复");
                            $(".orders-list>li[order-id="+ orderId +"]>div.order-content>div.order-text>ul").append("<li>房东回复:&nbsp;&nbsp;&nbsp;&nbsp;"+reply_text+"</li>");
